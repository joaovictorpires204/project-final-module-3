...
class AuthService {
  ...
  async auth(data: authUserDto) {
    ...
    if (!user) {
      return commonError("Email or password is not valid", STATUS_CODE.BAD_REQUEST);
    }

    const validatePassword = bcrypt.compareSync(data.password,(user as IUser).password as string);
